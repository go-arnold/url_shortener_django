...
class ShortURLCreateAPITests(APITestCase):
    ...
    def test_create_short_url_success(self):
        response = self.client.post(
            "/api/urls/", {"original_url": "https://example.com/path"}, format="json"
        )
        ...
        self.assertTrue(
            ShortURL.objects.filter(short_code=response.data["short_code"]).exists()
        )

    def test_create_short_url_rejects_invalid_url(self):
        response = self.client.post(
            "/api/urls/", {"original_url": "not-a-valid-url"}, format="json"
        )
    ...
    def test_create_short_url_returns_503_when_unique_code_cannot_be_generated(
        self, _mock_generate
    ):
        ShortURL.objects.create(
            original_url="https://example.com/existing", short_code="Ab12Cd"
        )

        response = self.client.post(
            "/api/urls/", {"original_url": "https://example.com/new"}, format="json"
        )
